...
class DeleteGuideUseCase @Inject constructor(
    ...
    operator fun invoke(guideDomainModel: GuideDomainModel, listImages: List<QuestionContentDomain.Image>): DeleteGuideResult {
        val deleteGuide = guiaRepository.deleteGuide(guideDomainModel, navigationPathRepository.currentPathGuides)
        if (!deleteGuide){
